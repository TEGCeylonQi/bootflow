...
**当前状态：v0.1.1** — 仍然**只读**，不修改系统的任何配置。程序启动后会自己检查有没有新版本。
...
- **更新检测**：启动后自己问一次"有没有新版本"，顶栏有常驻入口。有新版本时给出更新说明
  与产物清单，点一下打开发布页；也可以「忽略此版本」，下一个版本发布时会重新提醒。
  **检查失败时如实说"没问到"**，不谎报「已是最新」。联网走系统自带的 WinHTTP，
  自动跟随系统代理，因此通常不需要额外配置
...
版本号刻意走得小。`1.0.0` 的含义是**做完了**，不是"看起来成熟了"，
所以它留给下面全部内容落地、数据格式与接口冻结的那一天。

| 版本 | 内容 | 原则 |
| --- | --- | --- |
| **v0.1.0** | 只读体检：全来源发现 + 诊断 + 报告 | 零风险，先建立信任 |
| **v0.1.1** | 更新检测：启动后自动检查新版本，顶栏常驻入口 | 只检测，不自动替换安装 |
| **v0.2.0** | 启用/禁用开关、变更前快照、Dry-run 预演、一键回滚 | 任何写操作都可逆 |
| **v0.3.0** | **新建启动项**：从本机已装应用里选，自动避开已有项 | 只往可逆的落点写 |
| **v0.4.0** | 可视化编排：泳道拖拽、依赖图、分级延迟 | 从「管理单个项」到「编排整体时序」 |
| **v0.5.0** | 调度引擎：就绪探针、DAG 执行、编译器 | 让编排真正生效 |
| **v0.6.0** | IO 感知调度、一键优化建议、使用频率分析 | 软件替用户做决策 |
| **v0.7.0** | CLI、配置导入导出、便携模式、场景预设 | 能被别的工具调用 |
| **v1.0.0** | 接口冻结的稳定版 | 做完了，而不是看起来成熟了 |
...
```bash
# 1. 改三处版本号，改完立刻验一遍（npm run check 会检查它们是否一致）
...
三处必须一致：安装包版本来自 `tauri.conf.json`，而程序里显示的"当前版本"读的是
`Cargo.toml` 编进二进制的那个。不一致时，用户刚装上的新版会被它自己的更新检测
反复提示成"有新版本"。`tools/check-version.mjs` 专门拦这件事。
