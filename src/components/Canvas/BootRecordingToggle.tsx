 ...
 *
 * 所以它是 opt-in，且**关掉之后其余功能照常工作**：「每项在开机后第几秒出现」
 * 来自进程采样，不需要自启条目、也不需要任何权限。这个区别必须写在界面上，
 * 否则用户会以为关掉开关就什么都看不到了。
 *
 * ## 状态的唯一来源是后端返回值
 *
  ...
  return (
    <div className="rounded-card border border-line bg-base px-3 py-2.5">
      <div className="flex flex-wrap items-center gap-x-2 gap-y-1">
        ...
        <span className="text-2xs text-ink-dim">可选 · 默认关闭</span>
      ...
      <p className="mt-1.5 text-2xs leading-5 text-ink-muted">
        开启后会在系统里登记一个<span className="text-ink">自启条目</span>
        （已提权时为登录计划任务，否则为当前用户的注册表启动项），每次登录静默记下开机总时长，
        写进 <span className="text-ink-dim">%LOCALAPPDATA%\BootFlow\boot-records.json</span>
        ——不弹窗、不联网、不采集其他信息；关掉时把该条目一并移除。
        它只影响上面那条<span className="text-ink-muted">总时长曲线</span>：
        「每项在开机后第几秒出现」<span className="text-ink-muted">不依赖这个开关</span>。
      </p>
